...
INSERT INTO `t_order_detail` VALUES (3, NULL, 14, 1, 2290, 1000, 1290, 0, 0, 0, 1);
INSERT INTO `t_order_detail` VALUES (4, NULL, 12, 2, 4380, 2000, 2380, 0, 0, 0, 1);
INSERT INTO `t_order_detail` VALUES (5, NULL, 21, 1, 3000, 1500, 1500, 0, 0, 0, 1);
INSERT INTO `t_order_detail` VALUES (6, NULL, 12, 1, 2190, 1000, 1190, 0, 0, 0, 1);
INSERT INTO `t_order_detail` VALUES (7, NULL, 12, 1, 2190, 1000, 1190, 0, 0, 0, 1);
INSERT INTO `t_order_detail` VALUES (8, NULL, 12, 5, 10950, 5000, 5950, 0, 0, 0, 1);
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`localhost` SQL SECURITY DEFINER VIEW `q_order_detail` AS SELECT
	`t_order_detail`.`ID_order_detail` AS `ID_order_detail`,
	`t_order_detail`.`ID_order` AS `ID_order`,
	`t_cover`.`ID_pro` AS `ID_pro`,
	`t_product`.`p_name` AS `p_name`,
	`t_product`.`ID_type` AS `ID_type`,
	`t_type`.`t_name` AS `t_name`,
	`t_product`.`ID_art` AS `ID_art`,
	`t_artist`.`a_name` AS `a_name`,
	`t_cover`.`ID_web` AS `ID_web`,
	`t_website`.`w_name` AS `w_name`,
	`t_cover`.`ID_ver` AS `ID_ver`,
	`t_version`.`v_name` AS `v_name`,
	`t_order_detail`.`ID_cover` AS `ID_cover`,
	`t_cover`.`c_name` AS `c_name`,
	FORMAT(`t_cover`.`c_price_total`, 0) AS `c_price_total`,
	FORMAT(`t_cover`.`c_price_pledge`, 0) AS `c_price_pledge`,
	FORMAT(`t_cover`.`c_price_balance`, 0) AS `c_price_balance`,
	FORMAT(`t_cover`.`c_price_1st`, 0) AS `c_price_1st`,
	FORMAT(`t_cover`.`c_price_2nd`, 0) AS `c_price_2nd`,
	FORMAT(`t_cover`.`c_price_last`, 0) AS `c_price_last`,
	`t_order_detail`.`od_qty` AS `od_qty`,
	FORMAT(`t_order_detail`.`od_price_total`, 0) AS `od_price_total`,
	FORMAT(`t_order_detail`.`od_price_pledge`, 0) AS `od_price_pledge`,
	FORMAT(`t_order_detail`.`od_price_balance`, 0) AS `od_price_balance`,
	FORMAT(`t_order_detail`.`od_price_1st`, 0) AS `od_price_1st`,
	FORMAT(`t_order_detail`.`od_price_2nd`, 0) AS `od_price_2nd`,
	FORMAT(`t_order_detail`.`od_price_last`, 0) AS `od_price_last`,
	`t_product`.`ID_pay_type` AS `ID_pay_type`,
	`t_product`.`p_second_pay_date` AS `p_second_pay_date`,
	`t_product`.`p_last_pay_date` AS `p_last_pay_date`,
	`t_order_detail`.`ID_order_status` AS `ID_order_status`,
	`t_order_status`.`os_name` AS `os_name` 
FROM `t_order_detail`
JOIN `t_cover` ON `t_cover`.`ID_cover` = `t_order_detail`.`ID_cover`
JOIN `t_product` ON `t_product`.`ID_product` = `t_cover`.`ID_pro`
JOIN `t_website` ON `t_website`.`ID_web` = `t_cover`.`ID_web`
JOIN `t_version` ON `t_version`.`ID_ver` = `t_cover`.`ID_ver`
JOIN `t_type` ON `t_type`.`ID_type` = `t_product`.`ID_type`
JOIN `t_artist` ON `t_artist`.`ID_art` = `t_product`.`ID_art`
JOIN `t_order_status` ON `t_order_status`.`ID_order_status` = `t_order_detail`.`ID_order_status` ;
